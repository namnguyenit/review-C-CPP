...
## Mục Lục
- [1. Tại sao cần BusyBox? Thiếu gì? Cơ chế hoạt động](#1-tại-sao-cần-busybox-thiếu-gì-cơ-chế-hoạt-động)
- [2. Các bước khắc phục và giải thích build_rootfs.sh](#2-các-bước-khắc-phục-và-giải-thích-build_rootfssh)
- [3. Thay đổi cấu hình kernel và module](#3-thay-đổi-cấu-hình-kernel-và-module)

---

## 1. Tại sao cần BusyBox? Thiếu gì? Cơ chế hoạt động
...
### 1.3 Init Process là gì? Tại sao thiếu nó kernel sẽ panic?

**Init process** là **chương trình đầu tiên** mà kernel chạy sau khi boot xong. Nó có **PID = 1** (Process ID số 1) và là "cha" của mọi tiến trình khác trong hệ thống.

#### Kernel tìm init theo thứ tự:

```
Kernel boot xong → Tìm chương trình để chạy:
    1. /init              ← Ưu tiên cao nhất
    2. /sbin/init
    3. /bin/init
    4. /bin/sh             ← Phương án cuối cùng
    ↓
Không tìm thấy cái nào? → Kernel Panic!
```

#### Bản chất vấn đề: Thiếu phần cơ bản nhất của Userspace

**Đúng vậy** — cái thiếu chính xác là **môi trường dòng lệnh (userspace)**. Kernel **chỉ biết làm 2 việc**:

1. Quản lý phần cứng (CPU, RAM, disk, mạng...)
2. Tìm và chạy **đúng 1 chương trình** — init process

**Sau đó kernel không làm gì nữa.** Mọi thứ còn lại (hiển thị terminal, chạy lệnh, nạp module) đều do **userspace** đảm nhiệm.

| | Kernel | Userspace (init + shell) |
|---|---|---|
| **Ví von** | Bộ não | Tay, chân, miệng |
| **Vai trò** | Quản lý bộ nhớ, CPU, thiết bị | Cho phép **con người tương tác** |
| **Có thể tự chạy lệnh?** | ❌ Không | ✅ Có (`ls`, `cat`, `insmod`...) |
| **Có shell?** | ❌ Không | ✅ Có (`/bin/sh`) |

#### So sánh dễ hiểu

```
bzImage (kernel) = Hệ điều hành không có ứng dụng nào
                   Giống máy tính mới cài Windows nhưng xóa hết .exe
                   → Bật lên chỉ thấy màn hình đen, không gõ được gì

Kernel alone     = Bộ não không có cơ thể     → không làm được gì
Kernel + BusyBox = Bộ não + cơ thể tối thiểu → có shell, gõ lệnh được  
Kernel + Ubuntu  = Bộ não + cơ thể đầy đủ    → có GUI, network, package manager...
```

#### Init process trên các hệ thống khác nhau

| Hệ thống | Init process (PID 1) | Giải thích |
|---|---|---|
| **Ubuntu/Debian** | `systemd` | Khởi chạy hàng trăm service, rồi mở terminal |
| **Android** | `/init` (Android Init) | Khởi chạy `zygote`, `surfaceflinger`, rồi mở launcher |
| **Máy ảo QEMU của mình** | `/init` (BusyBox script) | Mount filesystem, rồi chạy `sh` cho gõ lệnh |

> [!IMPORTANT]
> Init process (PID 1) **không bao giờ được phép chết**. Nếu nó thoát → kernel panic ngay lập tức.
> Đó là lý do trong script `/init` ta dùng `exec /bin/sh -i` — lệnh `exec` **thay thế** process init
> bằng shell, chứ không chạy shell như subprocess. Nếu dùng `/bin/sh` không có `exec`, khi shell
> thoát, init cũng thoát theo → kernel panic.

### 1.4 Tại sao chọn BusyBox?
...
### 1.6 Chuỗi lỗi đã gặp và nguyên nhân gốc
